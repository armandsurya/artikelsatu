...
import { ChevronDown, GripVertical, Plus, Trash2, Upload, X, Image as ImageIcon, Search } from "lucide-react";
...
}: { label: string; hint?: string; error?: string; required?: boolean; children: ReactNode; counter?: string }) {
  return (
    <div className="block">
...
export function TextField({
  label, value, onChange, placeholder, required, hint, max, type = "text",
}: {
  label: string; value: string; onChange: (v: string) => void; placeholder?: string;
  required?: boolean; hint?: string; max?: number; type?: string;
}) {
  const over = max && value.length > max;
  return (
    <Field label={label} hint={hint} required={required} counter={max ? `${value.length}/${max}` : undefined}
...
export function TextareaField({
  label, value, onChange, rows = 4, required, hint, max, placeholder,
}: {
  label: string; value: string; onChange: (v: string) => void; rows?: number;
  required?: boolean; hint?: string; max?: number; placeholder?: string;
}) {
  const over = max && value.length > max;
  return (
    <Field label={label} hint={hint} required={required} counter={max ? `${value.length}/${max}` : undefined}
...
export function Accordion({ items }: {
  items: { key: string; title: string; subtitle?: string; badge?: ReactNode; right?: ReactNode; content: ReactNode; defaultOpen?: boolean }[];
}) {
            ...
            <li key={it.key}>
              ...
              >
                <span className="flex-1">
                  ...
                  {it.subtitle && <span className="mt-0.5 block text-xs text-muted-foreground">{it.subtitle}</span>}
                </span>
                {it.badge}
...
export function Repeater<T>({
  items, onChange, addLabel, newItem, renderItem, itemTitle,
}: {
  items: T[];
      ...
      {items.map((it, idx) => (
        <div key={idx} className="rounded-lg border border-border bg-background">
          <div className="flex items-center gap-2 border-b border-border px-4 py-2.5">
            <button type="button" onClick={() => move(idx, idx - 1)} className="rounded p-1 text-muted-foreground hover:bg-accent" aria-label="Naik">
              <GripVertical className="h-4 w-4" />
            </button>
            <span className="flex-1 text-sm font-medium text-secondary">
              {itemTitle ? itemTitle(it, idx) : `Item #${idx + 1}`}
            </span>
            <button type="button" onClick={() => move(idx, idx - 1)} className="rounded px-2 py-1 text-xs text-muted-foreground hover:bg-accent">↑</button>
            <button type="button" onClick={() => move(idx, idx + 1)} className="rounded px-2 py-1 text-xs text-muted-foreground hover:bg-accent">↓</button>
            <button
              type="button"
          ...
          <div className="p-4">
            {renderItem(it, (patch) => onChange(items.map((v, i) => (i === idx ? { ...v, ...patch } : v))), idx)}
          </div>
        </div>
      ))}
      <button
...
const ICON_CHOICES = [
  "Search", "BookOpen", "LayoutTemplate", "MessageSquare", "ShoppingBag", "RefreshCw",
  "Users", "PenTool", "ShieldCheck", "Target", "Clock", "TrendingDown", "TrendingUp",
  "UserX", "FileText", "PenLine", "Star", "CheckCircle2", "Award", "Zap", "Rocket",
  "Sparkles", "BarChart3", "Globe", "Heart", "ThumbsUp", "MessageCircle", "Mail",
  "Phone", "MapPin", "Calendar", "Layers", "Package", "Briefcase", "Lightbulb",
];

export function IconPicker({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  const [open, setOpen] = useState(false);
  const [q, setQ] = useState("");
  const Current = (Lucide as unknown as Record<string, React.ComponentType<{ className?: string }>>)[value] ?? Lucide.Sparkles;
      ...
      {open && (
        <Modal title="Pilih Icon" onClose={() => setOpen(false)}>
          <div className="mb-3 flex items-center gap-2 rounded-lg border border-border px-3 py-2">
            ...
            <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Cari icon…" className="flex-1 bg-transparent text-sm outline-none" />
          </div>
          <div className="grid max-h-[400px] grid-cols-6 gap-2 overflow-y-auto pr-1 sm:grid-cols-8">
            {filtered.map((n) => {
              const I = (Lucide as unknown as Record<string, React.ComponentType<{ className?: string }>>)[n];
              if (!I) return null;
              return (
                <button
                  key={n}
      ...
      <div className="flex items-center gap-3">
        {value ? (
          <div className="relative h-20 w-20 overflow-hidden rounded-lg border border-border bg-muted">
            ...
            <button type="button" onClick={() => onChange("")} className="absolute top-1 right-1 rounded-full bg-white/90 p-0.5 shadow" aria-label="Hapus">
        ...
        <div className="flex flex-col gap-2">
          <button type="button" onClick={() => setOpen(true)} className="inline-flex items-center gap-2 rounded-lg border border-border bg-background px-3 py-1.5 text-sm hover:bg-accent">
            <ImageIcon className="h-4 w-4" /> Pilih dari Media
          </button>
          {value && <span className="max-w-[280px] truncate text-xs text-muted-foreground">{value}</span>}
        </div>
      </div>
      {open && <MediaLibraryModal onClose={() => setOpen(false)} onPick={(url) => { onChange(url); setOpen(false); }} />}
    </Field>
  );
}
...
export function Modal({ title, children, onClose, wide }: { title: string; children: ReactNode; onClose: () => void; wide?: boolean }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={onClose}>
      <div className={`w-full ${wide ? "max-w-4xl" : "max-w-lg"} rounded-2xl bg-background shadow-xl`} onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between border-b border-border px-5 py-3.5">
          <h3 className="text-base font-semibold text-secondary">{title}</h3>
          <button onClick={onClose} className="rounded p-1 hover:bg-accent" aria-label="Tutup"><X className="h-4 w-4" /></button>
        </div>
...
export function SaveStatus({ state, savedAt }: { state: "idle" | "saving" | "saved" | "error"; savedAt?: string }) {
  ...
  if (state === "saved" || savedAt) return <span className="text-xs text-muted-foreground">Tersimpan{savedAt ? ` • ${new Date(savedAt).toLocaleString("id-ID")}` : ""}</span>;
  return null;
}
