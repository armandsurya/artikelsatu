...
export type HeroFormData = {
  ...
  secondaryButtonCustomUrl?: string;
  image: string;
  imageAlt: string;
  imageWidth: number;
  imageHeight: number;
...
export function HeroForm({
  ...
  value: HeroFormData;
  onChange: (v: HeroFormData) => void;
}) {
  const set = <K extends keyof HeroFormData>(k: K, v: HeroFormData[K]) =>
    onChange({ ...value, [k]: v });
  return (
    <div className="grid gap-6">
      <TextField
        ...
        max={60}
        placeholder="Contoh: Jasa Artikel SEO Berkualitas"
      />
      <TextField
        label="Headline"
        required
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <SelectField
          ...
          value={value.secondaryButtonTarget ?? "#pricing"}
          onChange={(v) => set("secondaryButtonTarget", v)}
          options={[
      ...
      {value.secondaryButtonTarget === "custom" && (
        <TextField
          label="Custom URL"
          value={value.secondaryButtonCustomUrl ?? ""}
          onChange={(v) => set("secondaryButtonCustomUrl", v)}
          placeholder="https://…"
        />
      )}
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <NumberField
          ...
          value={value.imageWidth ?? 1024}
          onChange={(v) => set("imageWidth", v)}
        />
        <NumberField
          label="Image Height (px)"
          value={value.imageHeight ?? 1024}
...
export function StatsForm({
  ...
  value: { items: StatItem[] };
  onChange: (v: { items: StatItem[] }) => void;
}) {
  return (
    <Repeater<StatItem>
      items={value.items ?? []}
      ...
      newItem={() => ({
        ...
        value: "",
        description: "",
        sortOrder: (value.items?.length ?? 0) + 1,
        isVisible: true,
      })}
      renderItem={(it, up) => (
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <NumberField
            label="Urutan"
            value={it.sortOrder}
            onChange={(sortOrder) => up({ sortOrder })}
          />
          <Field label="Visibility">
            <Switch
              checked={it.isVisible}
              onChange={(isVisible) => up({ isVisible })}
              label={it.isVisible ? "Tampil" : "Disembunyikan"}
            />
          </Field>
        </div>
      )}
...
export function ProblemsForm({
  ...
  value: { items: ProblemItem[] };
  onChange: (v: { items: ProblemItem[] }) => void;
}) {
  return (
    <Repeater<ProblemItem>
      ...
      renderItem={(it, up) => (
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <Field label="Visibility">
            <Switch checked={it.isVisible} onChange={(isVisible) => up({ isVisible })} />
          </Field>
...
export function SolutionsForm({
  ...
  value: { items: SolutionRow[] };
  onChange: (v: { items: SolutionRow[] }) => void;
}) {
  return (
    <Repeater<SolutionRow>
      ...
      renderItem={(it, up) => (
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <TextField
            label="Kolom Kiri (Regular)"
            value={it.regular}
            onChange={(regular) => up({ regular })}
            max={120}
          />
          <TextField
            label="Kolom Kanan (SEO)"
            value={it.seo}
            onChange={(seo) => up({ seo })}
            max={120}
          />
...
export function WorkflowForm({
  ...
  value: { items: WorkflowItem[] };
  onChange: (v: { items: WorkflowItem[] }) => void;
}) {
  return (
    <Repeater<WorkflowItem>
      ...
      newItem={() => ({
        stepNumber: (value.items?.length ?? 0) + 1,
        title: "",
        description: "",
        icon: "CheckCircle2",
        isVisible: true,
      ...
      renderItem={(it, up) => (
        <div className="grid gap-4 md:grid-cols-2">
          <NumberField
            label="Nomor"
            value={it.stepNumber}
            onChange={(stepNumber) => up({ stepNumber })}
          />
          <Field label="Icon">
            <IconPicker value={it.icon} onChange={(icon) => up({ icon })} />
          </Field>
...
export function AdvantagesForm({
  ...
  value: { items: AdvantageItem[] };
  onChange: (v: { items: AdvantageItem[] }) => void;
}) {
  return (
...
export function ServicesForm({
  ...
  value: { items: ServiceItem[] };
  onChange: (v: { items: ServiceItem[] }) => void;
}) {
  return (
      ...
      renderItem={(it, up) => (
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <TextField
            label="Slug"
            value={it.slug}
            onChange={(slug) => up({ slug: slug.toLowerCase().replace(/\s+/g, "-") })}
          />
          <TextField
            label="CTA Label"
            value={it.ctaLabel}
            onChange={(ctaLabel) => up({ ctaLabel })}
            max={30}
          />
          <div className="md:col-span-2">
            <TextareaField
...
export function PortfolioForm({
  ...
  value: { items: PortfolioItem[] };
  onChange: (v: { items: PortfolioItem[] }) => void;
}) {
  return (
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div className="md:col-span-2">
            <TextareaField
              ...
              value={it.excerpt ?? ""}
              onChange={(excerpt) => up({ excerpt })}
              max={120}
              rows={2}
              hint="Maksimal 120 karakter, tampil 2 baris."
            />
          </div>
          <TextField
            label="Keyword Utama"
            value={it.keyword}
            onChange={(keyword) => up({ keyword })}
            max={80}
          />
          <NumberField
            label="Jumlah Kata"
            value={it.wordCount}
            onChange={(wordCount) => up({ wordCount })}
          />
          <TextField
            label="CTA Text"
            required
            ...
            onChange={(ctaLabel) => up({ ctaLabel })}
            max={30}
            placeholder="Lihat Preview"
          />
          <TextField
            label="CTA URL"
            required
            value={it.ctaUrl ?? ""}
            onChange={(ctaUrl) => up({ ctaUrl })}
            placeholder="https://…"
          />
          <NumberField
            label="Urutan"
          ...
          <div className="md:col-span-2">
            <Field
              label="Label Tambahan (maks 3)"
              hint="Contoh: SEO Optimized, Human Written, Google Friendly."
            >
                ...
                {(it.labels ?? []).slice(0, 3).map((l, idx) => (
                  <div key={idx} className="flex gap-2">
                    ...
                    <button
                      ...
                      onClick={() =>
                        up({ labels: (it.labels ?? []).filter((_, i2) => i2 !== idx) })
                      }
                    >
                      Hapus
                    </button>
...
export function PricingForm({
  ...
  value: { items: PricingItem[] };
  onChange: (v: { items: PricingItem[] }) => void;
}) {
  return (
      ...
      renderItem={(it, up) => (
        <div className="grid gap-4 md:grid-cols-2">
          <TextField
            ...
            onChange={(packageName) => up({ packageName })}
            max={40}
          />
          <TextField
            label="Harga"
            value={it.price}
            onChange={(price) => up({ price })}
            placeholder="Rp 75rb"
          />
          <TextField
            label="Harga (catatan)"
            value={it.priceNote}
            onChange={(priceNote) => up({ priceNote })}
            placeholder="/artikel"
          />
          <TextField
            label="Badge"
            value={it.badge}
            onChange={(badge) => up({ badge })}
            placeholder="Terpopuler"
          />
          <Field label="Popular">
            <Switch checked={it.isPopular} onChange={(isPopular) => up({ isPopular })} />
          </Field>
          <Field label="Visibility">
            <Switch checked={it.isVisible} onChange={(isVisible) => up({ isVisible })} />
          </Field>
          <div className="md:col-span-2">
            <Field label="Fitur">
              <div className="space-y-2">
                {it.features.map((f, i) => (
                  <div key={i} className="flex items-center gap-2">
                    <input
                      ...
                      onChange={(e) =>
                        up({
                          features: it.features.map((x, j) =>
                            j === i ? { text: e.target.value } : x,
                          ),
                        })
                      }
                      className={inputCls}
                    ...
                    <button
                      type="button"
                      onClick={() => up({ features: it.features.filter((_, j) => j !== i) })}
                      className="rounded-lg border border-border px-2 py-1.5 text-xs text-red-600 hover:bg-red-50"
                    >
                      Hapus
                    </button>
                  </div>
                ...
                <button
                  type="button"
                  onClick={() => up({ features: [...it.features, { text: "" }] })}
                  className="rounded-lg border border-dashed border-border px-3 py-1.5 text-xs font-medium text-primary hover:bg-accent"
                >
                  + Tambah Fitur
                </button>
              </div>
...
export function ComparisonForm({
  ...
  value: { rows: ComparisonRow[] };
  onChange: (v: { rows: ComparisonRow[] }) => void;
}) {
  const rows = value.rows ?? [];
  const update = (i: number, patch: Partial<ComparisonRow>) =>
    onChange({ rows: rows.map((r, j) => (j === i ? { ...r, ...patch } : r)) });
  const remove = (i: number) => onChange({ rows: rows.filter((_, j) => j !== i) });
  const add = () =>
    onChange({ rows: [...rows, { feature: "", freelancer: "", ai: "", agency: "", us: "" }] });
  return (
    <div className="space-y-3">
          ...
          <thead className="bg-muted/40 text-left">
            <tr>
              {["Kolom", "Freelancer", "AI", "Agency", "Kami", ""].map((h) => (
                <th key={h} className="px-3 py-2 font-medium text-secondary">
                  {h}
                </th>
              ))}
            </tr>
              ...
              <tr key={i}>
                {(["feature", "freelancer", "ai", "agency", "us"] as const).map((k) => (
                  <td key={k} className="p-2">
                    <input
                      value={r[k]}
                      onChange={(e) => update(i, { [k]: e.target.value } as Partial<ComparisonRow>)}
                      className={inputCls}
                    />
                  </td>
                ))}
      ...
      <button
        type="button"
        onClick={add}
        className="inline-flex items-center gap-2 rounded-lg border border-dashed border-border px-4 py-2 text-sm font-medium text-primary hover:bg-accent"
      >
        + Tambah Baris
      </button>
    </div>
      ...
      renderItem={(it, up) => (
        <div className="grid gap-4">
          <TextField
            label="Pertanyaan"
            value={it.question}
            onChange={(question) => up({ question })}
            max={160}
          />
          <TextareaField
            label="Jawaban"
            value={it.answer}
            onChange={(answer) => up({ answer })}
            rows={3}
            max={600}
          />
          <div className="grid gap-4 md:grid-cols-2">
...
export function BlogPreviewForm({
  ...
  value: BlogPreviewData;
  onChange: (v: BlogPreviewData) => void;
}) {
  return (
    <div className="grid gap-6">
      <TextField
        label="Judul Section"
        value={value.sectionTitle ?? "Artikel Terbaru"}
        onChange={(sectionTitle) => onChange({ ...value, sectionTitle })}
        max={80}
      />
      <SelectField
        label="Jumlah Artikel"
        value={String(value.count ?? "3")}
