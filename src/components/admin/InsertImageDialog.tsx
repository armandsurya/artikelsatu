...
export function InsertImageDialog({
  ...
  return (
    <Modal title="Insert Image" onClose={onClose} wide>
      ...
      {tab === "library" && (
        <LibraryTab onPick={(m) => onInsert({ url: m.url, alt: m.alt ?? undefined })} />
      )}
      {tab === "upload" && (
        <UploadTab onDone={(m) => onInsert({ url: m.url, alt: m.alt ?? undefined })} />
      )}
  ...
  return (
    <div>
      ...
      <p className="mt-3 text-[11px] text-muted-foreground">
        Tip: klik untuk memilih, double-click untuk langsung insert.
      </p>
    </div>
  );
}
...
function UploadTab({ onDone }: { onDone: (m: MediaItem) => void }) {
  ...
  async function handleFiles(files: FileList | null) {
    ...
    onDone({
      ...
      size_bytes: res.media.size_bytes,
      ...
      >
        {uploading ? (
          <>
            ...
            <p className="text-sm text-secondary">Mengunggah…</p>
          </>
        ) : (
          <>
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                JPG, PNG, WebP, SVG, GIF, AVIF · maks 5MB
              </p>
            </div>
          </>
        )}
...
function UrlTab({
  ...
  onInsert: (p: { url: string; alt?: string; caption?: string }) => void;
}) {
