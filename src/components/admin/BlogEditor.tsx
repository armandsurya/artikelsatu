...
export function BlogEditor({ mode, id, onSaved }: Props) {
  ...
  const [busy, setBusy] = useState<
    null | "draft" | "publish" | "unpublish" | "duplicate" | "schedule" | "archive" | "restore"
  >(null);
  ...
  function writePreviewDraft() {
    ...
    try {
      ...
      console.warn("preview draft write failed", e);
    }
    return payload;
  }
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      if (e.key === "Escape") setQuickPreviewOpen(false);
    };
  ...
  const dirty = useMemo(
    ...
    [
      ...
      currentSnap.excerpt,
      currentSnap.content,
      currentSnap.featuredImage,
      currentSnap.imageAlt,
      currentSnap.imageCaption,
      currentSnap.categoryId,
      currentSnap.metaTitle,
      currentSnap.metaDesc,
      currentSnap.canonical,
      currentSnap.ogTitle,
      currentSnap.ogDesc,
      currentSnap.robotsIndex,
      currentSnap.tags,
      currentSnap.focusKeyword,
      currentSnap.publishedAt,
      currentSnap.scheduledAt,
      snapshot,
    ],
  );
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data, error }) => {
        if (error) {
          setToast({ kind: "err", msg: `Gagal memuat: ${error.message}` });
          setLoading(false);
          return;
        }
        if (!data) {
        ...
        setScheduledAt(loaded.scheduledAt);
  ...
  useEffect(() => {
    if (stats.minutes && stats.minutes !== readTime)
    ...
    shouldBlockFn: () => {
      if (bypassGuardRef.current) {
        ...
        if (import.meta.env.DEV)
          console.debug("[BlogEditor] guard bypassed after successful save/publish");
        return false;
      }
  ...
  async function persist(
    ...
    if (featuredImage)
      await trackMediaUsage(featuredImage, "blog_post", saved.id, "featured_image");
    ...
    await logActivity(
      ...
      "blog_posts",
      saved.id,
      { title: payload.title, status: nextStatus },
    );
  ...
  async function handle(
    action: "draft" | "publish" | "unpublish" | "schedule" | "archive" | "restore",
  ) {
    setBusy(action);
    setToast(null);
    try {
      ...
      if (import.meta.env.DEV)
        console.debug("[BlogEditor] persisted → snapshot reset", { action, saved: res });
      ...
      const msg =
        action === "publish"
          ...
          : action === "schedule"
            ? "Jadwal publish tersimpan"
            : action === "unpublish"
              ? "Artikel dikembalikan menjadi Draft"
              : action === "archive"
                ? "Artikel diarsipkan"
                : action === "restore"
                  ? "Artikel dikembalikan dari arsip menjadi Draft"
  ...
  return (
    <div>
      ...
      <div className="z-20 -mx-6 mb-4 border-b border-border bg-background/95 px-6 py-3 backdrop-blur">
        ...
        {toast && (
          <div
            className={`mt-3 flex items-center gap-2 rounded-md border px-3 py-2 text-sm ${toast.kind === "ok" ? "border-green-200 bg-green-50 text-green-800" : "border-red-200 bg-red-50 text-red-800"}`}
          >
            {toast.kind === "ok" ? (
        ...
        <div className="mx-auto w-full max-w-none space-y-4">
          <Field label="Judul Artikel">
            <input
              ...
              className={inputCls}
              placeholder="Judul artikel yang menarik & mengandung keyword"
            />
          </Field>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Field label="Tags" hint="Pisahkan dengan koma">
              <input
                ...
                className={inputCls}
                placeholder="seo, menulis"
              />
            </Field>
          </div>
          <Field
            label="Excerpt"
            hint="Ringkasan singkat (muncul di daftar & default meta description)"
          >
            <textarea
              value={excerpt}
          ...
          <EditorLiveStats
            ...
            readTimeOverride={readTime}
            onOverrideChange={setReadTime}
          />
        </div>
      )}
      ...
      {tab === "media" && (
        <div className="mx-auto max-w-4xl space-y-4">
          <Card>
            ...
            {featuredImage && (
              <FeaturedMediaMetadataEditor url={featuredImage} articleTitle={title} />
            )}
          </Card>
          <Card>
            <h3 className="mb-1 text-sm font-semibold text-secondary">Gallery</h3>
            <p className="text-xs text-muted-foreground">
              Fitur gallery akan tersedia pada rilis berikutnya. Sisipkan gambar tambahan langsung
              di editor konten via toolbar Tiptap.
            </p>
          </Card>
        </div>
      )}
          ...
          <div className="space-y-4">
            ...
            <Card>
              ...
              <div className="space-y-3">
                ...
                <p className="text-xs text-muted-foreground">
                  Twitter Card otomatis <code>summary_large_image</code>. OG Image otomatis
            ...
            <Card>
              ...
              <p className="mt-2 text-xs text-muted-foreground">
                Schema <code>Article</code> JSON-LD dihasilkan otomatis pada halaman detail.
              </p>
            </Card>
          </div>
          <div className="space-y-4">
            <Card>
              <h3 className="mb-3 flex items-center gap-1.5 text-sm font-semibold text-secondary">
                <FileText className="h-4 w-4" /> Checklist Publish
              </h3>
              <ul className="space-y-1.5 text-xs">
                {[
                  ...
                  <li
                    key={label as string}
                    className={`flex items-center gap-2 ${ok ? "text-green-700" : "text-muted-foreground"}`}
                  >
                    {ok ? (
            ...
            <Card>
              ...
              <div
                className={`mb-3 flex items-center justify-between rounded-md border px-3 py-2 ${bandColor}`}
              >
        ...
        <div className="mx-auto max-w-3xl space-y-4">
          <Card>
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Author diambil dari akun pembuat artikel. Untuk multi-author, gunakan fitur transfer
              author (akan datang).
            </p>
          </Card>
          <Card>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <Field label="Status Saat Ini">
                <div
                  className={`inline-flex items-center rounded-md border px-3 py-2 text-sm font-medium ${STATUS_STYLES[status]}`}
                >
                  {status.toUpperCase()}
                </div>
              </Field>
          ...
          <Card>
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Semua artikel menggunakan template dinamis <code>/blog/$slug</code>. Halaman detail
              terbentuk otomatis saat artikel Published.
            </p>
          </Card>
        </div>
      )}
        ...
        <div className="mx-auto max-w-4xl">
          <RevisionsPanel
            ...
            onRestore={async (revId) => {
              ...
              setToast({
                kind: "ok",
                msg: `Revisi #${rev.revision_number} dimuat. Klik Save Draft untuk menyimpan.`,
              });
            }}
          />
        </div>
        ...
        <div className="mx-auto max-w-4xl">
          <Card>
            <p className="text-sm text-muted-foreground">
              Simpan artikel sebagai Draft terlebih dahulu untuk melihat riwayat revisi.
            </p>
          </Card>
        </div>
      )}
    ...
    return (
      <div className="rounded-md border border-border p-3">
        <div className="mb-2 flex items-center justify-between">
          ...
          <span className={`text-[11px] ${same ? "text-muted-foreground" : "text-amber-600"}`}>
            {same ? "sama" : "berbeda"}
          </span>
        </div>
        <div className="grid gap-2 sm:grid-cols-2">
          <div>
            ...
            <div className="max-h-40 overflow-y-auto whitespace-pre-wrap text-xs text-secondary">
              {past || "—"}
            </div>
          </div>
          <div>
      ...
      <Card className="!p-0">
        ...
        <div className="max-h-[560px] overflow-y-auto p-2">
          ...
          {!isLoading && revs.length === 0 && (
            <div className="p-4 text-sm text-muted-foreground">Belum ada revisi.</div>
          )}
      ...
      <Card>
        {!compareId && (
          <div className="text-sm text-muted-foreground">
            Pilih revisi di kiri untuk membandingkan dengan konten saat ini.
          </div>
        )}
        {compareId && compareQ.isLoading && (
          <div className="text-sm text-muted-foreground">Memuat revisi…</div>
        )}
        {compareId && compareQ.data && (
          <div className="space-y-3">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div>
                <h4 className="text-sm font-semibold text-secondary">
                  Revisi #{compareQ.data.revision_number}
                </h4>
                <p className="text-[11px] text-muted-foreground">
                  {new Date(compareQ.data.created_at).toLocaleString("id-ID")} · status:{" "}
                  {compareQ.data.status ?? "—"}
                </p>
              </div>
              <button onClick={() => onRestore(compareId)} className={btnPrimary}>
                <RotateCcw className="h-4 w-4" /> Restore Revisi Ini
              </button>
            </div>
            <div className="space-y-2">
              ...
              {diffField(
                "Meta Description",
                currentSnapshot.metaDesc,
                compareQ.data.meta_description ?? "",
              )}
              {diffField(
                "Konten",
                currentSnapshot.content,
                typeof compareQ.data.content === "string" ? compareQ.data.content : "",
              )}
            </div>
          </div>
        )}
...
function FeaturedMediaMetadataEditor({ url, articleTitle }: { url: string; articleTitle: string }) {
  ...
  if (!media) {
    return (
      <div className="mt-3 rounded-md border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800">
        Gambar ini tidak berasal dari Media Library sehingga metadata tidak dapat dikelola. Upload
        ulang melalui Media Library atau ganti gambar untuk mengaktifkan pengelolaan ALT & Caption.
      </div>
    );
  }
  ...
  async function save() {
    setErr(null);
    if (over) {
      setErr("Melebihi batas karakter.");
      return;
    }
    setSaving(true);
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        <Field label={`Title (${title.length}/${LIMITS.title})`}>
          <input
            ...
            className={inputCls + (editing ? "" : " opacity-70")}
          />
        </Field>
        <Field
          label={`ALT Text (${alt.length}/${LIMITS.alt})`}
          hint="Wajib untuk SEO & aksesibilitas"
        >
          <input
            disabled={!editing}
            value={alt}
            onChange={(e) => setAlt(e.target.value)}
            maxLength={LIMITS.alt + 20}
            className={inputCls + (editing ? "" : " opacity-70")}
            placeholder={articleTitle || "Deskripsi singkat gambar"}
          />
        </Field>
        <Field
          label={`Caption (${caption.length}/${LIMITS.caption})`}
          hint="Ditampilkan di bawah gambar pada frontend"
        >
          <input
            disabled={!editing}
            value={caption}
            onChange={(e) => setCaption(e.target.value)}
            maxLength={LIMITS.caption + 20}
            className={inputCls + (editing ? "" : " opacity-70")}
          />
        </Field>
...
function StatCell({
  ...
  return (
    <div
      className={`rounded-md border px-2.5 py-2 ${warn ? "border-amber-300 bg-amber-50" : "border-border bg-background"}`}
    >
      <div className={`text-[15px] font-semibold ${warn ? "text-amber-800" : "text-secondary"}`}>
        {value}
      </div>
      <div
        className="mt-0.5 truncate text-[10.5px] uppercase tracking-wide text-muted-foreground"
        title={hint}
      >
        {label}
      </div>
