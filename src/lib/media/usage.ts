...
export async function trackMediaUsage(
  ...
  await supabase.from("media_usage").delete().eq("context", context).eq("context_id", contextId).eq("field", field);
...
export async function clearMediaUsage(context: UsageContext, contextId: string, field?: string): Promise<void> {
...
export async function getMediaUsage(mediaId: string): Promise<UsageRow[]> {
  const { data } = await supabase.from("media_usage").select("*").eq("media_id", mediaId).order("created_at");
