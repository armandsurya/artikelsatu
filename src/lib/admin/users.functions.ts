...
export const listUsers = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { apiAdmin } = await import("@/integrations/supabase/client.server");
    const { data: authList, error: authErr } = await apiAdmin.auth.admin.listUsers({
      page: 1,
      perPage: 200,
    });
...
export const inviteUser = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    let userId: string;
    if (data.password) {
      const { data: created, error } = await apiAdmin.auth.admin.createUser({
        email: data.email,
      ...
      const { data: invited, error } = await apiAdmin.auth.admin.inviteUserByEmail(
        data.email,
        {
    ...
    if (data.roles.length) {
      await apiAdmin.from("user_roles").upsert(
...
export const deleteUser = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const { error } = await apiAdmin.auth.admin.deleteUser(data.id);
    if (error) throw new Error(error.message);
    return { ok: true };
  });
...
export const sendPasswordReset = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    await requireSuperAdmin(context as never);
    const { apiAdmin } = await import("@/integrations/supabase/client.server");
    const { error } = await apiAdmin.auth.admin.generateLink({
      type: "recovery",
      email: data.email,
    });
    if (error) throw new Error(error.message);
...
export const toggleBan = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const { apiAdmin } = await import("@/integrations/supabase/client.server");
    const { error } = await apiAdmin.auth.admin.updateUserById(data.id, {
