...
export async function patchSiteSettings(
  ...
  const { data: row, error: readErr } = await supabase
    ...
    .maybeSingle();
  if (readErr) {
    console.error("[siteSettings] read failed", readErr);
  ...
  if (!row) {
    const { data, error } = await supabase
      ...
      .select();
    if (error) {
      console.error("[siteSettings] insert failed", error);
      return { error: { message: error.message, code: error.code, hint: error.hint ?? undefined } };
    }
    if (!data || data.length === 0)
      return {
        error: {
          message:
            "Tidak ada baris yang tersimpan. Akun Anda kemungkinan tidak memiliki hak akses (super_admin) atau session sudah kadaluarsa. Silakan login ulang.",
        },
      };
    return { error: null };
  }
  ...
  if (!data || data.length === 0)
    return {
      error: {
        message:
          "Perubahan tidak tersimpan (0 baris ter-update). Session Anda kemungkinan kadaluarsa atau akun bukan super_admin. Silakan login ulang.",
      },
    };
  return { error: null };
}
