...
export const getPublishedPostBySlug = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }): Promise<{ payload: string | null }> => {
    ...
    const { data: row, error } = await client
      ...
      .is("deleted_at", null)
      .maybeSingle();
    if (error) { console.error("[getPublishedPostBySlug]", error); return { payload: null }; }
    return { payload: row ? JSON.stringify(row) : null };
  });

export const listPublishedPosts = createServerFn({ method: "GET" }).handler(async (): Promise<{ payload: string }> => {
  const client = getClient();
  ...
  if (error) { console.error("[listPublishedPosts]", error); return { payload: "[]" }; }
  return { payload: JSON.stringify(data ?? []) };
});

export const listBlogCategories = createServerFn({ method: "GET" }).handler(async (): Promise<{ payload: string }> => {
  const client = getClient();
  if (!client) return { payload: "[]" };
  const { data, error } = await client
    ...
    .select("id,name,slug")
    .order("name");
  if (error) { console.error("[listBlogCategories]", error); return { payload: "[]" }; }
  return { payload: JSON.stringify(data ?? []) };
});
