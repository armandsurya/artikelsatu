...
function PreviewPage() {
  ...
  const [draft, setDraft] = useState<DraftPayload | null>(() => (isDraftSentinel ? readDraft() : null));
  ...
  const { data: catName } = useQuery({
    ...
    queryFn: async () => (await supabase.from("blog_categories").select("name").eq("id", draft!.category_id!).maybeSingle()).data?.name ?? null,
  });
  ...
  const post = draft
    ...
    ? {
        ...
        category_name: (dbPost as { blog_categories?: { name?: string } }).blog_categories?.name ?? "Umum",
      }
    : null;
  ...
  const { data: authorName } = useQuery({
    ...
    queryFn: async () => (await supabase.from("profiles").select("full_name").eq("id", post!.author_id!).maybeSingle()).data?.full_name ?? null,
  });

  if (isLoading && !post) return <SiteLayout><div className="container-narrow py-20 text-center text-sm text-muted-foreground">Memuat preview…</div></SiteLayout>;
  if (!post) return (
    <SiteLayout>
  ...
  const date = post.published_at
    ...
    : new Date(post.updated_at).toLocaleDateString("id-ID", { day: "numeric", month: "long", year: "numeric" });
      ...
      <div className="sticky top-0 z-30 border-b border-amber-200 bg-amber-50">
        <div className="container-narrow flex items-center justify-between py-2 text-xs text-amber-900">
          <span className="inline-flex items-center gap-1.5 font-medium">
            ...
            {draft && <span className="ml-2 inline-flex items-center gap-1 rounded-full bg-amber-200/60 px-1.5 py-0.5 text-[10px] font-semibold uppercase"><RefreshCw className="h-3 w-3" /> Live Draft</span>}
      ...
      <article className="bg-background">
        <div className="container-narrow py-14">
          ...
          <div className="mt-6 max-w-3xl">
            <span className="inline-flex rounded-full bg-accent px-2.5 py-1 text-xs font-medium text-accent-foreground">{post.category_name}</span>
            ...
            <div className="mt-6 flex flex-wrap items-center gap-x-5 gap-y-2 text-sm text-muted-foreground">
              <span className="inline-flex items-center gap-1.5"><User className="h-4 w-4" />{authorName ?? "Tim ArtikelPro"}</span>
