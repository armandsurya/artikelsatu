...
function LogPage() {
  const { data: rows = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      return (logs ?? []).map((l) => ({ ...l, userName: l.user_id ? nameMap.get(l.user_id) ?? "—" : "sistem" }));
    },
  });

  return (
    <div>
      <PageHeader title="Log Aktivitas" description="Catatan login, edit, upload, dan perubahan pengaturan." />
      <Card className="!p-0 overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-border">
            {rows.length === 0 && <tr><td colSpan={5} className="px-4 py-10 text-center text-muted-foreground">Belum ada aktivitas.</td></tr>}
            {rows.map((r) => (
              <tr key={r.id}>
