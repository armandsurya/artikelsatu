...
function RedirectPage() {
  ...
  return (
    <div>
      <PageHeader
        title="Redirect URL"
        description="Kelola redirect 301 & 302 lengkap dengan pelacakan kunjungan."
      />
      <div className="grid gap-6 lg:grid-cols-[360px_1fr]">
        <Card>
          ...
          <div className="space-y-3">
            ...
            <Field label="Destination URL">
              <input
                ...
                className={inputCls}
                placeholder="/halaman-baru atau https://..."
              />
            </Field>
            <Field label="Code">
              <select
            ...
            <button
              onClick={add}
              disabled={saving || !source || !destination}
          ...
          <div className="mt-4 rounded-md border border-border/60 bg-muted/30 p-3 text-xs text-muted-foreground">
            Tips: gunakan path relatif diawali <code>/</code> untuk redirect internal. Loop otomatis
            dicegah. Source case-insensitive dan unik.
          </div>
        </Card>
                  ...
                  return (
                    <tr key={r.id} className={isEditing ? "bg-muted/30" : ""}>
                      <td className="px-4 py-3 font-mono text-xs">
                        {isEditing ? (
                          <input
                            value={(editDraft.source ?? r.source) as string}
                            onChange={(e) =>
                      ...
                      <td className="px-4 py-3 font-mono text-xs">
                        {isEditing ? (
                          <input
                            value={(editDraft.destination ?? r.destination) as string}
                            onChange={(e) =>
                      ...
                      <td className="px-4 py-3">
                        {isEditing ? (
                          <select
                            value={(editDraft.code ?? r.code) as number}
                            onChange={(e) =>
                              setEditDraft((d) => ({
                                ...d,
                                code: Number(e.target.value) as 301 | 302,
                              }))
                            }
                            className={inputCls + " !py-1 !text-xs"}
                          >
                      ...
                      <td
                        ...
                        title={r.last_hit_at ? "Terakhir: " + fmt(r.last_hit_at) : ""}
                      >
                        ...
                        <div className="flex justify-end gap-1">
                          {isEditing ? (
                            <>
                              ...
                              <button
                                onClick={cancelEdit}
                                className="inline-flex items-center rounded border border-border px-2 py-1 text-xs"
                              >
