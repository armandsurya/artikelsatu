...
import {
  ...
  Repeater,
  inputCls,
} from "@/components/admin/homepage/primitives";
...
function FooterEditor() {
  ...
  useEffect(() => {
    (async () => {
      const all = await loadSiteSettings<{
        footer?: FooterData;
        footer_draft?: FooterData;
        footer_saved_at?: string;
      }>();
  ...
  async function saveDraft(): Promise<boolean> {
    ...
    try {
      ...
      if (error) {
        console.error("[footer saveDraft]", error);
  ...
  async function publish() {
    ...
    try {
      ...
      if (error) {
        console.error("[footer publish]", error);
        toast.error("Gagal mem-publish footer", { description: error.message });
        return;
      }
      setLive(local);
  ...
  function openPreview() {
    window.open(`/?preview=footer&t=${Date.now()}`, "_blank", "noopener,noreferrer");
  }

  if (!local) return <div className="text-sm text-muted-foreground">Memuat…</div>;
  const set = <K extends keyof FooterData>(k: K, v: FooterData[K]) =>
    setLocal({ ...local, [k]: v });
      ...
      <Card>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <TextField
            ...
            onChange={(v) => set("contact", { ...local.contact, whatsapp: v })}
                ...
                <div className="space-y-2">
                  {col.links.map((l, i) => (
                    <div key={i} className="grid gap-2 md:grid-cols-[1fr_1fr_auto]">
                      ...
                      <button
                        type="button"
                        onClick={() => upCol({ links: col.links.filter((_, j) => j !== i) })}
                        className="rounded-lg border border-border px-3 py-1.5 text-xs text-red-600 hover:bg-red-50"
                      >
                        Hapus
                      </button>
                    </div>
                  ...
                  <button
                    type="button"
                    onClick={() => upCol({ links: [...col.links, { label: "", href: "" }] })}
