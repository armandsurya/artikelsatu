...
function validate(s: PengaturanUmum): string | null {
  ...
  if (s.whatsapp && !/^\d{8,15}$/.test(s.whatsapp))
    return "Nomor WhatsApp harus 8-15 digit angka (tanpa + atau spasi).";
  for (const sm of s.social ?? []) {
    if (sm.url && !/^https?:\/\//i.test(sm.url))
      return `URL social "${sm.label || sm.url}" harus dimulai dengan http(s)://`;
  }
  return null;
}
...
function Pengaturan() {
  ...
  async function save() {
    ...
    const currentSeo = ((data?.seo as Record<string, unknown> | undefined) ?? {}) as Record<
      string,
      unknown
    >;
    ...
    toast.success("Pengaturan tersimpan", {
      description: "Perubahan sudah aktif di seluruh website.",
    });
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <Card>
          ...
          <div className="space-y-3">
            ...
            <Field label="Email">
              <input {...bind("email")} className={inputCls} type="email" />
            </Field>
            <Field label="Alamat">
              <textarea {...bind("address")} rows={2} className={inputCls} />
            </Field>
          </div>
        </Card>
          ...
          <div className="space-y-2">
            {social.length === 0 && (
              <p className="text-sm text-muted-foreground">
                Belum ada social media. Klik "+ Tambah".
              </p>
            )}
            {social.map((sm, i) => (
              <div key={i} className="flex gap-2">
                <input
                  placeholder="Label (Instagram, LinkedIn, …)"
                  value={sm.label}
        ...
        <Card className="md:col-span-2 border-dashed bg-muted/30">
          <p className="text-sm text-muted-foreground">
            <strong className="text-secondary">Catatan arsitektur:</strong> konfigurasi Meta Title,
            Meta Description, dan Analytics (Google Analytics, Search Console, dsb.) telah dipindah
            ke menu <strong>SEO</strong>. Data lama Anda sudah otomatis dimigrasi — silakan cek menu
            SEO untuk mengelolanya.
          </p>
        </Card>
      </div>
    </div>
