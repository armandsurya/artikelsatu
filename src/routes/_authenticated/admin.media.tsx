...
import {
  Upload, Search, Trash2, Copy, X, Download, Check, ImageOff, RefreshCw,
} from "lucide-react";
import { supabase } from "@/integrations/supabase/client";
import { PageHeader, Card, inputCls, btnPrimary, btnGhost, btnDanger, Field } from "@/components/admin/ui";
import { logActivity } from "@/lib/admin/log";
import {
  uploadMediaFile, replaceMediaFile, validateFile, formatBytes, ALLOWED_MIME,
...
type MediaRow = {
  ...
  alt: string | null; title: string | null; caption: string | null; description: string | null;
...
function MediaLibraryPage() {
  ...
  async function handleFiles(files: FileList | File[]) {
    ...
    for (const f of list) {
      ...
      if (invalid) { toast.error(`${f.name}: ${invalid.message}`); fail++; continue; }
      const res = await uploadMediaFile(f);
      if (res.ok) { ok++; await logActivity("upload_media", "media", res.media.id, { name: res.media.name }); }
      else { toast.error(`${f.name} (${res.step}): ${res.message}`); fail++; }
    }
    setUploading(false);
  ...
  async function bulkDownload() {
    ...
    for (const r of items) {
      ...
      a.href = r.url; a.download = r.name; a.target = "_blank"; a.rel = "noopener";
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={
          <>
            <input ref={fileRef} type="file" multiple hidden accept={acceptAttr}
      ...
      <Card className="mb-4">
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative min-w-[220px] flex-1">
            ...
            <input value={q} onChange={(e) => { setQ(e.target.value); setPage(0); }} placeholder="Cari nama, alt, title…" className={`${inputCls} pl-9`} />
          </div>
          <select value={typeFilter} onChange={(e) => { setTypeFilter(e.target.value); setPage(0); }} className={inputCls + " max-w-[180px]"}>
          ...
          <select value={sort} onChange={(e) => setSort(e.target.value as SortKey)} className={inputCls + " max-w-[180px]"}>
        ...
        {selection.size > 0 && (
          <div className="mt-3 flex items-center gap-2 rounded-lg border border-primary/30 bg-primary/5 px-3 py-2 text-sm">
            ...
            <button onClick={bulkDownload} className={btnGhost + " !py-1.5 text-xs"}><Download className="h-3.5 w-3.5" /> Download</button>
            <button onClick={bulkDelete} className={btnDanger + " text-xs"}><Trash2 className="h-3.5 w-3.5" /> Hapus</button>
            <button onClick={() => setSelection(new Set())} className={btnGhost + " !py-1.5 text-xs"}>Batal</button>
          </div>
        )}
      </Card>
      ...
      {isLoading ? (
        ...
        <Card><p className="py-12 text-center text-sm text-muted-foreground">Belum ada media yang cocok.</p></Card>
      ) : (
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">
          {paged.map((m) => {
            const isSel = selection.has(m.id);
            return (
              <div key={m.id} className={`group relative overflow-hidden rounded-xl border bg-background transition-shadow hover:shadow-md ${isSel ? "border-primary ring-2 ring-primary/30" : "border-border"}`}>
                <button
                  type="button"
                ...
                <button type="button" onClick={() => setSelected(m)} className="block w-full text-left">
                  ...
                  <div className="p-2.5">
                    <div className="truncate text-xs font-medium text-secondary" title={m.name}>{m.name}</div>
                    <div className="mt-0.5 flex items-center justify-between text-[10px] text-muted-foreground">
                      <span>{m.width && m.height ? `${m.width}×${m.height}` : "-"}</span>
                      <span>{formatBytes(m.size_bytes)}</span>
                    </div>
                    <div className="mt-1 flex items-center justify-between text-[10px] text-muted-foreground">
                      <span className="rounded bg-muted px-1.5 py-0.5">{m.mime_type?.split("/")[1] ?? "?"}</span>
      ...
      {pageCount > 1 && (
        <div className="mt-6 flex items-center justify-center gap-2">
          <button disabled={page === 0} onClick={() => setPage((p) => p - 1)} className={btnGhost + " disabled:opacity-40"}>‹ Prev</button>
          <span className="text-sm text-muted-foreground">Halaman {page + 1} / {pageCount}</span>
          <button disabled={page + 1 >= pageCount} onClick={() => setPage((p) => p + 1)} className={btnGhost + " disabled:opacity-40"}>Next ›</button>
        </div>
      )}

      {selected && <DetailDrawer media={selected} onClose={() => setSelected(null)} onChanged={() => qc.invalidateQueries({ queryKey: ["media"] })} />}
    </div>
  );
}
...
function DetailDrawer({ media, onClose, onChanged }: { media: MediaRow; onClose: () => void; onChanged: () => void }) {
  ...
  useState(() => { getMediaUsage(media.id).then(setUsage); });
  ...
  async function onReplace(file: File) {
    ...
    setReplacing(false);
    if (!res.ok) { toast.error(`Replace gagal (${res.step})`, { description: res.message }); return; }
  ...
  return (
    <div className="fixed inset-0 z-50 flex" onClick={onClose}>
      ...
      <aside onClick={(e) => e.stopPropagation()} className="flex h-full w-full max-w-xl flex-col overflow-y-auto bg-background shadow-2xl">
        ...
        <div className="space-y-5 p-5">
          <div className="overflow-hidden rounded-lg border border-border bg-muted">
            {media.mime_type?.startsWith("image/") ? (
              <img src={media.url} alt={alt || media.name} className="max-h-[300px] w-full object-contain" />
            ) : (
              <div className="flex h-40 items-center justify-center text-muted-foreground">Preview tidak tersedia</div>
            )}
          </div>

          <div className="grid grid-cols-2 gap-3 text-xs">
            <Info label="Dimensi" value={media.width && media.height ? `${media.width} × ${media.height} px` : "-"} />
          ...
          <div className="space-y-3 rounded-lg border border-border p-4">
            <Field label="Nama File"><input value={name} onChange={(e) => setName(e.target.value)} className={inputCls} /></Field>
            <Field label="ALT" hint="Wajib untuk SEO & aksesibilitas."><input value={alt} onChange={(e) => setAlt(e.target.value)} className={inputCls} /></Field>
            <Field label="Title"><input value={title} onChange={(e) => setTitle(e.target.value)} className={inputCls} /></Field>
            <Field label="Caption"><input value={caption} onChange={(e) => setCaption(e.target.value)} className={inputCls} /></Field>
            ...
            <button onClick={save} disabled={saving} className={btnPrimary + " w-full justify-center"}>{saving ? "Menyimpan…" : "Save"}</button>
          </div>
          ...
          <div className="rounded-lg border border-border p-4">
            ...
            {usage === null ? (
              ...
              <ul className="space-y-1 text-xs">
                {usage.map((u) => <li key={u.id} className="rounded bg-muted px-2 py-1">{describeUsage(u)}</li>)}
              </ul>
            )}
          </div>

          <div className="flex gap-2">
            <input ref={replaceRef} type="file" hidden accept={acceptAttr} onChange={(e) => e.target.files?.[0] && onReplace(e.target.files[0])} />
            <button onClick={() => replaceRef.current?.click()} disabled={replacing} className={btnGhost + " flex-1 justify-center"}>
...
function Info({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded border border-border bg-muted/30 px-2.5 py-1.5">
      ...
      <div className="mt-0.5 truncate text-secondary" title={value}>{value}</div>
    </div>
  );
}
...
function CopyRow({ label, value, onCopy }: { label: string; value: string; onCopy: () => void }) {
  return (
    <div className="flex items-center gap-2 rounded border border-border bg-muted/20 px-2.5 py-1.5 text-xs">
      ...
      <span className="flex-1 truncate text-secondary" title={value}>{value}</span>
      <button onClick={onCopy} className="rounded p-1 text-muted-foreground hover:bg-accent hover:text-secondary" aria-label="Copy">
